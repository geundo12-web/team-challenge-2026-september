...
function bindEvents() {
  els.refreshButton.addEventListener("click", () => refreshFromServer({ silent: false }));
  els.exportButton.addEventListener("click", exportJson);
  els.importInput.addEventListener("change", importJson);
  els.clearLocalButton.addEventListener("click", clearLocalRecords);
  ...
  els.noteInput.addEventListener("input", () => {
    ...
    els.noteCounter.textContent = `${els.noteInput.value.length} / 200`;
  });
  els.saveEditorButton.addEventListener("click", saveEditorRecord);
  els.deleteRecordButton.addEventListener("click", deleteEditorRecord);
...
function render() {
  ...
  els.todayText.textContent = formatKoreanDate(today);
  els.ddayText.textContent = getDdayText(today);
  renderSyncStatus();
  renderMissionCards();
  renderRecordGrid();
  renderRankings();
...
function renderSyncStatus() {
  ...
  if (hasSync) {
    ...
    els.syncStatusText.textContent = "공동 저장 모드";
    ...
    .map((participant) => {
      ...
      return `
        <article class="mission-card">
          ...
          <div class="sub-info">
            <span>연속 성공일 ${streak}일</span>
            ${isAutoPassDate(today, participant.id) ? "<span>주말 자동 패스</span>" : ""}
          </div>
...
function renderRankings() {
  ...
  els.teamFineText.textContent = formatMoney(teamFine);
